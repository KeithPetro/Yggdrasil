...
def importGedcom(path):
    ...
    for gedcom_filename in os.listdir(path):
        ...
        all_records = gedcom_parser.get_root_child_elements()

        for record in all_records:
            if record.get_tag() == "FAM":
                family_members_gedcom_p = gedcom_parser.get_family_members(
                    record, "PARENTS")

                family_members_gedcom_c = gedcom_parser.get_family_members(
                    record, "CHIL")
                ...
                for parent in family_members_gedcom_p:
                    (element, created) = Individual.objects.get_or_create(
                        ...
                        marriage_data=json.dumps(
                            gedcom_parser.get_marriages(parent)),
                ...
                for child in family_members_gedcom_c:
                    (element, created) = Individual.objects.get_or_create(
                        ...
                        marriage_data=json.dumps(
                            gedcom_parser.get_marriages(child)),
                        defaults={'last_change_date': datetime.date.today()},
                    )
